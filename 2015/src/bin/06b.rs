...
fn main() -> Result<()> {
    ...
    let mut lights = [0usize; WIDTH * HEIGHT];
    ...
    for line in input.lines() {
        ...
        for x in x0..=x1 {
            for y in y0..=y1 {
                ...
                match instruction {
                    TurnOn => {
                        lights[i] = lights[i].saturating_add(1);
                    }
                    TurnOff => {
                        lights[i] = lights[i].saturating_sub(1);
                    }
                    Toggle => {
                        lights[i] = lights[i].saturating_add(2);
                    }
                }
            }
        }
